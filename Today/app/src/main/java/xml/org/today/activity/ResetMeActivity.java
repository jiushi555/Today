 ...
 */
public class ResetMeActivity extends BaseActivity {
    private EditText mUsername;
    private TextView mBtnReset;
    private LinearLayout mPwd;
    private ImageView goback, portrait;
    private String url;
    private Context mContext = ResetMeActivity.this;
    // 头像参数
    ...
    private int flag_portrait = 0, flag_name = 0;  //标记修改的地方
    private int result_portrait = 0, result_name = 0;   //标记修改的地方是否成功
    private int finish_portrait = 0, finish_name = 0;   //标记异步是否加载完成
    private static final int NOHTTP_WHAT_TEST = 0x013;
    private static final int NOHTTP_WHAT_TEST2 = 0x016;
    private int FLAG=0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_resetme);
        initView();
    }

    private void initView() {
        portrait = (ImageView) findViewById(R.id.resetme_portrait);
        if (UserInfo.getUserImg(mContext) == null |
                BitmapFactory.decodeFile(UserInfo.getUserImg(mContext)) == null) {
            url = UserInfo.getUserPortraitUrl(ResetMeActivity.this);
            portrait.setTag(url);
            ...
            portrait.setImageBitmap(RoundBitmapUtil.toRoundBitmap(b));
        }
        goback = (ImageView) findViewById(R.id.resetme_goback);
        goback.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ResetMeActivity.this.finish();
            }
        });
        portrait.setOnClickListener(new PortraitOnClick());
        mUsername = (EditText) findViewById(R.id.resetme_username);
        mUsername.setText(UserInfo.getUserName(ResetMeActivity.this));
        mUsername.setSelection(mUsername.getText().length());
        mBtnReset = (TextView) findViewById(R.id.resetme_btn);
        mPwd = (LinearLayout) findViewById(R.id.resetme_pwd);
        mPwd.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent it = new Intent(ResetMeActivity.this, RlPwdActivity.class);
                startActivity(it);
                ResetMeActivity.this.finish();
            }
        });
        mBtnReset.setOnClickListener(new BtnOnClick());
    }
    ...
    private class BtnOnClick implements View.OnClickListener {
        ...
        public void onClick(View v) {
            ...
            if (username.equals(UserInfo.getUserName(mContext))) {
                flag_name = 0;
            } else {
                flag_name = 1;
            }
            if (flag_name == 0 && flag_portrait == 0) {
                Toast.makeText(ResetMeActivity.this, "您未修改任何内容", Toast.LENGTH_SHORT).show();
            } else {
                showDialog();
                if (flag_portrait == 1) {
                    uploadFile();
                ...
                if (flag_name == 1) {
                    ServerApi.getResetMeResult(NOHTTP_WHAT_TEST2, mContext, username, onResponseListener2);
                }
            }
        }
    }
    private void sendBoradcastReceiver(){
    ...
    private void checkResult() {
        if (flag_name == 1 && flag_portrait != 1) {
            closeDialog();
            if (result_name == 1) {
                Toast.makeText(ResetMeActivity.this, "修改成功", Toast.LENGTH_SHORT).show();
                UserInfo.setUsername(username, mContext);
                sendBoradcastReceiver();
            } else {
                Toast.makeText(ResetMeActivity.this, "修改失败", Toast.LENGTH_SHORT).show();
            }
        } else if (flag_portrait == 1 && flag_name != 1) {
            closeDialog();
            if (result_portrait == 1) {
                Toast.makeText(ResetMeActivity.this, "修改成功", Toast.LENGTH_SHORT).show();
                UserInfo.setUserImg(path, ResetMeActivity.this);
                sendBoradcastReceiver();
            } else {
                Toast.makeText(ResetMeActivity.this, "修改失败", Toast.LENGTH_SHORT).show();
            }

        } else if (flag_name == 1 && flag_portrait == 1) {
            if (finish_name == 1 && finish_portrait == 1) {
                closeDialog();
                if (result_portrait == 1 && result_name == 1) {
                    Toast.makeText(ResetMeActivity.this, "修改成功", Toast.LENGTH_SHORT).show();
                    UserInfo.setUsername(username, mContext);
                    UserInfo.setUserImg(path, ResetMeActivity.this);
                    sendBoradcastReceiver();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == FLAG_CHOOSE_IMG && resultCode == RESULT_OK) {
            ...
            if (data != null) {
                ...
                ResetMeActivity.this.path = path;
    ...
    private void uploadFile() {
        ...
        try {
            ...
            ServerApi.getUploadUserResult(0x013, filename, mContext, onResponseListener);
        } catch (Exception e) {
            result_portrait = 2;
        }
    ...
    protected void onDestroy() {
        if(FLAG==1){
            Intent it=new Intent(ResetMeActivity.this,MainActivity.class);
            startActivity(it);
        }
        super.onDestroy();
    }
